    ...
    <div className="sticky top-0 z-30 border-b border-[#F0E4E1] bg-[#F8F4F0]/85 backdrop-blur supports-[backdrop-filter]:bg-[#F8F4F0]/65">
      <div className="mx-auto flex max-w-[960px] items-center gap-4 px-6 py-3 max-sm:px-4">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          ...
          >
            <Button size="lg" className="hover:bg-primary/80 transition-colors cursor-pointer">
              Get Hemo
            </Button>
          </a>
        </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={cn(
            "mb-4 inline-flex items-center gap-2.5 text-[11px] font-semibold uppercase tracking-[1.6px]",
            eyebrowCls,
          )}
        >
          <span
            className={cn(
              ...
              isGradient ? "bg-[#F8E9E7]" : "bg-[#A9334D]",
              "animate-pulse",
            )}
          />
          AI summary · Generated by Hemo
        </div>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          <PeriodTabs current={data.periodDays} variant={style} />
          <span
            className={cn(
...
function BarColumn({
  ...
  return (
    <Card className="rounded-[20px] border-[#F0E4E1] bg-white p-6 shadow-[0_1px_8px_rgba(0,0,0,0.06)]">
      <div className="text-[11px] font-semibold uppercase tracking-[1.4px] text-[#1A1414]/65">
        {eyebrow}
      </div>
      <h3 className="mb-4 mt-1 text-[18px] font-bold tracking-[-0.4px] text-[#781D11]">
        {title}
      </h3>
      <div className="flex flex-col gap-3.5">
        {items.map((it) => (
          <div key={it.name} className="grid grid-cols-[1fr_auto] gap-1.5">
            <div className="text-[14px] font-medium tracking-[-0.1px] text-[#1A1414]">
              {it.name}
            </div>
            <div className="font-mono text-[12px] font-medium text-[#1A1414]/65">
              {it.count}×{" "}
              <span className="text-[#1A1414]/30">
                · {Math.round((it.count / periodDays) * 100)}%
              </span>
            </div>
            <div className="col-span-2 h-2 overflow-hidden rounded-full bg-[#F3EAE7]">
              <div
...
function PatternsSection({ data }: { data: HealthSummaryData }) {
  ...
  const max = Math.max(
    ...topSymptoms.map((s) => s.count),
    ...topTriggers.map((t) => t.count),
    1,
  );
...
function PrivacyBanner({
  ...
  const desc =
    anonymization === "full"
      ? "Full personal information visible as shared."
      : anonymization === "partial"
        ? "Name partially redacted. Hospital and blood type are hidden."
        : "Identified by initials only. All sensitive details hidden.";

  return (
    <div className="mx-auto mt-7 max-w-[960px] px-6 max-sm:px-4">
      <div
        className="flex items-center gap-3.5 rounded-[16px] border border-[#F0E4E1] border-l-[#A9334D] bg-white px-4 py-3.5 text-[13px] leading-[1.45] text-[#1A1414]/65"
        style={{ borderLeftColor: "#A9334D", borderLeftWidth: 4 }}
      >
        <Lock className="size-3.5 shrink-0 text-[#A9334D]" />
        <div>
          <b className="font-semibold text-[#781D11]">
            {first} chose what to share.
          </b>{" "}
          {desc} This link can be revoked at any time from the Hemo app.
        </div>
      </div>
    </div>
  );
...
function SummaryFooter({ data }: { data: HealthSummaryData }) {
  const first =
    data.profile?.nickname ||
    data.profile?.full_name?.split(" ")[0] ||
    "the patient";
  return (
    <footer className="mt-16 border-t border-[#F0E4E1] bg-[#F8F4F0] px-6 pb-14 pt-10 max-sm:px-4">
      <div className="mx-auto max-w-[960px]">
        <div className="grid grid-cols-[1fr_auto] items-center gap-6 max-sm:grid-cols-1">
          <div className="flex items-center gap-3.5">
            <span
              ...
              aria-hidden
            />
            <div className="max-w-[460px] text-[14px] leading-[1.5] tracking-[-0.1px] text-[#1A1414]/65">
          ...
          <div className="flex items-center gap-2.5">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 rounded-full border border-[#F0E4E1] bg-white px-3 py-1.5 text-[13px] font-semibold text-[#781D11] transition-colors hover:bg-[#F8E9E7]"
            >
              <ExternalLink className="size-3.5" /> View in Hemo
            </a>
            <a
              href="https://hemoscd.com"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center rounded-full bg-[#A9334D] px-3 py-1.5 text-[13px] font-semibold text-white transition-colors hover:bg-[#92293f]"
            >
              Get the app
            </a>
          </div>
        ...
        <Separator className="my-8 bg-[#F0E4E1]" />

        <p className="m-0 max-w-[640px] text-[11px] leading-[1.5] tracking-[0.1px] text-[#1A1414]/45">
          This summary was generated automatically from {first}'s daily logs in
          Hemo on {formatGeneratedAt(data.generatedAt)}. The patterns surfaced
          here are heuristic, not a clinical diagnosis. Always discuss findings
          with a licensed care provider before changing medication or treatment.
        </p>
      </div>
    </footer>
  );
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#FFF9F9] text-[#1A1414] antialiased">
      ...
      <main>
        ...
        <PrivacyBanner data={data} anonymization={anonymization} />
      </main>
