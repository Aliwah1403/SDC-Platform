...
function QuietAction({ icon: Icon, label, onPress, disabled = false, tint }) {
  return (
    ...
    >
      ...
      <Text
        style={{
          color: disabled ? tint.muted : tint.action,
  ...
  return (
    ...
    >
      <Pressable
        ...
        accessibilityRole="button"
        accessibilityLabel={
          location ? `${title}, ${location.name}` : `${title}, ${emptyText}`
        }
        style={({ pressed }) => ({
          padding: 16,
          gap: 7,
      ...
      >
        <View style={{ flexDirection: "row", alignItems: "center", gap: 7 }}>
          ...
          {location ? (
            ...
            <Plus size={18} color={t.textSecondary} />
          )}
        </View>
        <Text
          selectable
      ...
      {location ? (
        ...
        >
          ...
          <QuietAction
            icon={Navigation}
            label={mapActionLabel || "Directions unavailable"}
            onPress={onDirections}
            disabled={!mapActionLabel}
            tint={tint}
          />
        </View>
      ) : (
        ...
        >
          <QuietAction
            ...
            label="Set location"
            onPress={onAdd}
            tint={{ action: tone.color, muted: t.textTertiary }}
          />
        </View>
      )}
    </View>
...
export default function CareLocationsScreen() {
  ...
  const { saveNotice } = useLocalSearchParams();
  ...
  const others = locations.filter(
    (item) =>
      ![
        CARE_LOCATION_ROLES.PREFERRED_ED,
        CARE_LOCATION_ROLES.REGULAR_CLINIC,
      ].includes(item.role),
  );
  ...
  const shownSaveNotice = useRef(null);

  useEffect(() => {
    const notice = Array.isArray(saveNotice) ? saveNotice[0] : saveNotice;
    if (!notice || shownSaveNotice.current === notice) return;
    shownSaveNotice.current = notice;
    requestAnimationFrame(() => showToast({ message: "Care location saved" }));
  }, [saveNotice, showToast]);
  ...
  useEffect(() => {
    ...
    posthog?.capture("care_locations_opened", {
      entry_point: "care",
      has_preferred_ed: !!preferredED,
      has_regular_clinic: !!regularClinic,
      saved_count: locations.length,
    });
  }, [isLoading, locations.length, !!preferredED, !!regularClinic]);

  const openForm = (role, location) => {
    posthog?.capture("care_location_add_started", {
      entry_point: "care",
      selected_role: role,
    });
    router.push({
      pathname: "/(tabs)/care/care-location-form",
      params: { role, id: location?.id || "" },
    });
  };
  ...
  const confirmDelete = (location) =>
    Alert.alert("Delete care location?", location.name, [
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.background }}>
      <CareLocationsHeader
        ...
        subtitle={`${locations.length} saved ${locations.length === 1 ? "location" : "locations"}`}
        onBack={() => router.back()}
        actionIcon={<Plus size={22} color="#F8E9E7" strokeWidth={2.5} />}
        actionLabel="Add care location"
        onAction={() => openForm(CARE_LOCATION_ROLES.OTHER)}
      />
      <ScrollView
      ...
      >
        ...
        {isError ? (
          ...
          >
            <Text
              selectable
              style={{ color: t.text, fontFamily: fonts.semibold }}
            >
              Couldn’t load your care locations
            </Text>
            <Text
              selectable
              style={{
            ...
            >
              Tap to try again. No sample healthcare data will be shown.
            </Text>
          </Pressable>
        ) : null}
        ...
        <View style={{ gap: 10 }}>
          <PrimaryLocationCard
            ...
            emptyText="Add your preferred emergency department"
            location={preferredED}
            tone={ROLE_TONES.preferred_ed}
            onAdd={() => openForm(CARE_LOCATION_ROLES.PREFERRED_ED)}
            onEdit={() => openForm(preferredED.role, preferredED)}
            onCall={() => call(preferredED)}
            onDirections={() => directions(preferredED)}
          />
          <PrimaryLocationCard
            t={t}
            title="Regular care"
            emptyText="Add your regular SCD clinic"
            location={regularClinic}
            tone={ROLE_TONES.regular_scd_clinic}
            onAdd={() => openForm(CARE_LOCATION_ROLES.REGULAR_CLINIC)}
            onEdit={() => openForm(regularClinic.role, regularClinic)}
            onCall={() => call(regularClinic)}
            onDirections={() => directions(regularClinic)}
          />
        </View>
        ...
        <View style={{ gap: 10 }}>
          <Text
            selectable
            style={{ fontFamily: fonts.semibold, color: t.text, fontSize: 17 }}
          >
          ...
          {others.length === 0 ? (
            <Text
              selectable
              style={{ fontFamily: fonts.regular, color: t.textSecondary }}
            >
              Pharmacies, transfusion centres and other places you add will
              appear here.
            </Text>
          ) : (
            others.map((location) => {
              ...
              return (
                <Pressable
                  key={location.id}
                  onPress={() => openForm(location.role, location)}
                  onLongPress={() => confirmDelete(location)}
                  accessibilityLabel={`${location.name}, ${CARE_LOCATION_ROLE_LABELS[location.role]}`}
                  style={({ pressed }) => ({
                    backgroundColor: t.surface,
                    borderRadius: 17,
                    padding: 15,
                ...
                >
                  ...
                  >
                    <MapPin size={19} color={tone.color} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text
                      selectable
                    ...
                    >
                      {CARE_LOCATION_ROLE_LABELS[location.role]} · Added by you
                    </Text>
                  </View>
                  <ChevronRight size={18} color={t.textSecondary} />
                </Pressable>
        ...
        {pins.length > 0 ? (
          <Pressable
            onPress={() => {
              posthog?.capture("care_locations_map_opened", {
                pin_count: pins.length,
                filter: "saved",
              });
              router.push("/(tabs)/care/care-locations-map");
            }}
            accessibilityRole="button"
            style={{
              backgroundColor: t.surface,
          ...
          >
            <View pointerEvents="none">
              <CareLocationsMap locations={pins} preview />
            </View>
            <View
              style={{
                padding: 14,
            ...
            >
              <Map size={19} color={t.accent} />
              <Text
                style={{ flex: 1, fontFamily: fonts.semibold, color: t.text }}
              >
                View your saved locations map
              </Text>
        ...
        <View style={{ gap: 10 }}>
          ...
          >
            These searches open your maps app. Results are external and are not
            verified by Hemo for SCD care.
          </Text>
          {[
            ["Emergency departments", "emergency department"],
            ["Pharmacies", "pharmacy"],
          ].map(([label, query]) => (
            <Pressable
              key={query}
            ...
            >
              <ExternalLink size={18} color={t.accent} />
              <Text
                style={{ flex: 1, fontFamily: fonts.semibold, color: t.text }}
              >
                Search {label} in Maps
              </Text>
              <ChevronRight size={18} color={t.textSecondary} />
            </Pressable>
          ))}
