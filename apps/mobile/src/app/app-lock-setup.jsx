...
import { ArrowLeft, Check, Fingerprint, Lock, ShieldCheck } from 'lucide-react-native';
...
export default function AppLockSetupScreen() {
  ...
  const { appLockEnabled, appLockTimeout, setAppLockEnabled, setAppLockTimeout } = useAppStore();
  ...
  const handleGetStarted = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        Alert.alert('Authentication failed', 'Please try again to enable App Lock.');
      }
    } catch {
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      {step === 'intro' ? (
        <IntroStep loading={loading} onGetStarted={handleGetStarted} insets={insets} />
      ) : (
        <ConfigStep
  ...
  return (
    <View style={{ flex: 1, paddingHorizontal: 28 }}>
      ...
      <Text style={styles.body}>
        Add Face ID, Touch ID, or your device passcode so only you can access your health data — even if someone else picks up your phone.
      </Text>
      ...
      <Pressable
        style={({ pressed }) => [styles.primaryBtn, pressed && { opacity: 0.85 }, loading && { opacity: 0.7 }, { marginBottom: insets.bottom + 24 }]}
...
function ConfigStep({ selectedTimeout, onSelectTimeout, onSave, onDisable, isExisting, insets }) {
  return (
    <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: insets.bottom + 24 }}>
      ...
      <View style={styles.optionsCard}>
        {TIMEOUT_OPTIONS.map((opt, i) => (
          <View key={opt.value}>
            <Pressable
              style={({ pressed }) => [styles.optionRow, pressed && { backgroundColor: '#F8F4F0' }]}
      ...
      <Pressable
        style={({ pressed }) => [styles.primaryBtn, pressed && { opacity: 0.85 }, { marginTop: 24 }]}
        onPress={onSave}
      >
        <Text style={styles.primaryBtnText}>{isExisting ? 'Save changes' : 'Enable App Lock'}</Text>
      </Pressable>
