...
function SegmentedControl({ options, selected, onSelect, t }) {
...
export default function EditBodyStats() {
  ...
  const [totalInches, setTotalInches] = useState(67);
  ...
  useEffect(() => {
    if (profile && !initialized) {
      if (profile.height) {
        const cm = Math.min(CM_MAX, Math.max(CM_MIN, Math.round(profile.height)));
        setCmValue(cm);
        setTotalInches(Math.round(cm / 2.54));
      }
      if (profile.weight) {
        const kg = Math.min(KG_MAX, Math.max(KG_MIN, Math.round(profile.weight)));
        setKgValue(kg);
            ...
            >
              ...
              {!editingHeight && (
                <View style={styles.tapHint}>
                  ...
                  <PenLine size={14} color={t.textTertiary} strokeWidth={1.8} />
                </View>
              )}
            </Pressable>
                ...
                {heightUnit === "Metric" ? (
                  ...
                  >
                    {Array.from({ length: CM_MAX - CM_MIN + 1 }, (_, i) => {
                      const val = i + CM_MIN;
                      return <Picker.Item key={val} label={`${val} cm`} value={val} />;
                    })}
                  </Picker>
                ) : (
                  <View style={styles.imperialRow}>
                    ...
                    <Picker
                      ...
                      onValueChange={(inches) =>
                        setTotalInches(Math.floor(totalInches / 12) * 12 + inches)
                      }
                      style={styles.imperialPicker}
                      itemStyle={styles.pickerItem}
                    >
