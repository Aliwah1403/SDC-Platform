...
// A few mock suggestions for a realistic typeahead feel
...
export default function Step6() {
  ...
  const handleContinue = () => {
    if (selected || query.trim()) {
      setOnboardingField('preferredHospital', selected || query.trim());
    }
    router.push('/(onboarding)/step-7');
  };
  ...
  return (
    <OnboardingStep
      ...
      ctaLabel={selected || query.trim() ? 'Save & Continue' : 'Skip'}
    >
      {/* Search input */}
      ...
      {suggestions.length > 0 && focused && (
        <View style={styles.suggestions}>
          {suggestions.map((s) => (
            <Pressable
              key={s}
              style={({ pressed }) => [styles.suggestion, pressed && { backgroundColor: 'rgba(9,51,44,0.06)' }]}
      ...
      <View style={styles.infoCard}>
        <Text style={styles.infoText}>
          Can't find your hospital? Type the full name and tap "Save & Continue" — you can search from a map later in the Care Hub.
        </Text>
      </View>
    </OnboardingStep>
  );
